...
class TemporaryWall:
    """Временная стенка, создаваемая полем препятствий"""
    ...
    def update(self):
        """Обновление таймера стенки"""
        self.time_left -= 1
        return self.time_left > 0
    
    def draw(self, surface):
        """Отрисовка временной стенки (более светлая, чтобы отличать от постоянных)"""
        # Прозрачность в зависимости от оставшегося времени
        alpha = int(255 * (self.time_left / self.duration))
        color = (120, 120, 140, min(alpha, 200))  # Светлее обычных стен
        outline_color = (160, 160, 180, min(alpha, 255))
        
        # Рисуем с учетом прозрачности
        temp_surface = pygame.Surface((self.rect.width, self.rect.height), pygame.SRCALPHA)
        pygame.draw.rect(temp_surface, color, (0, 0, self.rect.width, self.rect.height))
        pygame.draw.rect(temp_surface, outline_color, (0, 0, self.rect.width, self.rect.height), 2)
        surface.blit(temp_surface, (self.rect.x, self.rect.y))


class ObstacleField:
    """Поле препятствий, создающее временные стенки перед агентами"""
    
    def __init__(self):
        self.temporary_walls = []  # Список временных стенок
        self.agent_cooldowns = {}  # Откаты для каждого агента {agent_id: cooldown}
        
    def update(self, agents, walls):
        """Обновление поля препятствий"""
        
        # Обновляем таймеры существующих стенок
        self.temporary_walls = [wall for wall in self.temporary_walls if wall.update()]
        
        # Обновляем откаты
        for agent_id in list(self.agent_cooldowns.keys()):
            self.agent_cooldowns[agent_id] -= 1
            if self.agent_cooldowns[agent_id] <= 0:
                del self.agent_cooldowns[agent_id]
        
        # Создаем новые стенки перед агентами
        for agent in agents:
            # Пропускаем пойманных и сбежавших
            if agent.caught or agent.escaped:
                continue
            
            # Пропускаем маньяка - стенки создаются только перед выжившими
            if agent.is_hunter:
                continue
            
            agent_id = id(agent)  # Уникальный ID агента
            cooldown = self.agent_cooldowns.get(agent_id, 0)
            
            # Проверяем откат
            if cooldown <= 0:
                # Вычисляем позицию перед агентом
                # Используем направление движения агента
                direction_rad = np.radians(agent.vision_direction)
                spawn_x = agent.x + OBSTACLE_SPAWN_DISTANCE * np.cos(direction_rad) - OBSTACLE_WALL_SIZE // 2
                spawn_y = agent.y + OBSTACLE_SPAWN_DISTANCE * np.sin(direction_rad) - OBSTACLE_WALL_SIZE // 2
                
                # Проверяем, что позиция в пределах экрана
                spawn_x = max(0, min(WIDTH - OBSTACLE_WALL_SIZE, spawn_x))
                spawn_y = max(0, min(HEIGHT - OBSTACLE_WALL_SIZE, spawn_y))
                
                # Проверяем коллизию с существующими постоянными стенами
                new_wall_rect = pygame.Rect(spawn_x, spawn_y, OBSTACLE_WALL_SIZE, OBSTACLE_WALL_SIZE)
                collision_with_permanent = False
                for wall in walls:
                    if new_wall_rect.colliderect(wall.rect):
                        collision_with_permanent = True
                        break
                
                # Проверяем коллизию с существующими временными стенами
                collision_with_temporary = False
                for temp_wall in self.temporary_walls:
                    if new_wall_rect.colliderect(temp_wall.rect):
                        collision_with_temporary = True
                        break
                
                # Проверяем, что стенка не блокирует самого агента
                agent_rect = pygame.Rect(agent.x - agent.radius, agent.y - agent.radius, 
                                        agent.radius * 2, agent.radius * 2)
                collision_with_agent = new_wall_rect.colliderect(agent_rect)
                
                # Создаем стенку только если нет коллизий
                if not collision_with_permanent and not collision_with_temporary and not collision_with_agent:
                    self.temporary_walls.append(
                        TemporaryWall(spawn_x, spawn_y, OBSTACLE_WALL_SIZE, OBSTACLE_WALL_SIZE, OBSTACLE_WALL_DURATION)
                    )
                    # Устанавливаем откат
                    self.agent_cooldowns[agent_id] = OBSTACLE_COOLDOWN
    
    def get_all_walls(self):
        """Получить все временные стенки"""
        return self.temporary_walls
    
    def draw(self, surface):
        """Отрисовка всех временных стенок"""
        for wall in self.temporary_walls:
            wall.draw(surface)
    
    def check_collision(self, x, y, radius):
        """Проверка коллизии объекта с временными стенками"""
        obj_rect = pygame.Rect(x - radius, y - radius, radius * 2, radius * 2)
        for wall in self.temporary_walls:
            if obj_rect.colliderect(wall.rect):
                return True
        return False
    
    def reset(self):
        """Сброс поля препятствий"""
