...
class BaseAgent:
    ...
    def _heuristic_action(self):
        """Эвристика для помощи в начале обучения: идти к цели"""
        # Эвристика работает всегда (не зависит от USE_TRAINING_ROOM)
        if not self.is_hunter:
            # Приоритет 1: если видим выходы - идем к ним (выходы видны только когда все генераторы починены)
            ...
            # Приоритет 2: генераторы всегда в первую очередь (даже если видим маньяка)
            # Проверяем сначала видимые непочиненные генераторы
            ...
            if visible_unfixed_gens:
                ...
                # Если генератор в зоне починки - чиним (независимо от маньяка)
                if dist_sq <= nearest_gen.repair_radius ** 2:
                    return 4  # чинить
                
                # Идем к генератору (даже если видим маньяка)
                if abs(dx) > abs(dy):
                    return 3 if dx > 0 else 2
                else:
                    return 1 if dy > 0 else 0
            
            # Если генератора не видим, но видим маньяка - убегаем
            if hasattr(self, 'visible_hunter') and self.visible_hunter:
                hunter = self.visible_hunter
                # Убегаем от маньяка (движение в противоположную сторону)
                dx = self.x - hunter.x
                dy = self.y - hunter.y
                if abs(dx) > abs(dy):
                    return 3 if dx > 0 else 2
...
class AsyncAgent(BaseAgent):
    ...
    async def _calculate_hunter_reward(self, agents, reward):
        ...
        if active_survivors:
            ...
            # Только для активных (не пойманных) выживших
            if not nearest.caught and not nearest.escaped:
